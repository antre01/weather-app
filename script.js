...
async function getFetchData(endPoint, city) {
  // Wywołaj Netlify Function zamiast bezpośrednio API
  const apiUrl = `/.netlify/functions/weather?endpoint=${endPoint}&city=${encodeURIComponent(city)}`;
...
function findTemperatureAtSpecificHour(forecastData, targetHour) {
  // Check if forecastData and list exist
...
async function updateWeatherInfo(city) {
  ...
  // Download and display the min/max temperature
  ...
  // Display the minimum temperature if the element exists
  if (minTempTxt && todayTemps.minTemp !== null) {
    minTempTxt.textContent = `${Math.round(todayTemps.minTemp)}°C`;
  }
